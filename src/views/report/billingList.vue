...
<script>
import { getBillingList, downloadBillingList } from '@/api/reports'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        'second.timeType': [{ required: true, message: '请选定时间范围类型', trigger: 'change' }],
  ...
  methods: {
    ...
    getReport() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.loading = true
          getBillingList({ first: [this.queryParam.first], second: [this.queryParam.second] }).then(response => {
    ...
    download() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          downloadBillingList({ first: [this.queryParam.first], second: [this.queryParam.second] }).then((res) => {
            if (!res) return
